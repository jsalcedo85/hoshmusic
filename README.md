...
[Invite the public Hosh bot](https://discord.com/oauth2/authorize?client_id=774043716797071371&permissions=277028620608&scope=applications.commands%20bot) • [Support Server](https://discord.gg/ACJQzJuckW) • [Hosh Web Dashboard](https://hosh.app) • [CodeShare](https://codeshare.me)
...
Hosh reads from both `config.js` defaults and environment variables via `.env`. Update whichever approach fits your hosting workflow.
...
2. Name your integration (e.g., `Hosh Bot`) and enable **Web API**.
...
Hosh uses **web scraping** by default to fetch lyrics from Genius—no API key required! However, if you want **higher rate limits** and **faster responses**, you can optionally add Genius API credentials.
...
When your bot reaches **1,000+ servers**, Discord **requires** you to use sharding to distribute the load across multiple processes. Hosh includes a fully automated sharding system powered by Discord.js's `ShardingManager`.
...
Hosh features an intelligent autoplay engine that keeps the music flowing when your queue runs out.
...
Hosh eliminates playback interruptions by pre-downloading and caching all audio locally before streaming to Discord.
